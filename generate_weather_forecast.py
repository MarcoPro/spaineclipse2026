    ...
    for i in range(0, len(grid_points), BATCH_SIZE):
        ...
        if batch_data and len(batch_data) == len(batch):
            for j, p_data in enumerate(batch_data):
                ...
                c_total = hourly.get("cloud_cover", [0])[idx] if hourly.get("cloud_cover") else 0
                c_low = hourly.get("cloud_cover_low", [0])[idx] if hourly.get("cloud_cover_low") else 0
                c_mid = hourly.get("cloud_cover_mid", [0])[idx] if hourly.get("cloud_cover_mid") else 0
                c_high = hourly.get("cloud_cover_high", [0])[idx] if hourly.get("cloud_cover_high") else 0
                precip = hourly.get("precipitation_probability", [0])[idx] if hourly.get("precipitation_probability") else 0
                w_code = hourly.get("weather_code", [0])[idx] if hourly.get("weather_code") else 0
                temp = hourly.get("temperature_2m", [25.0])[idx] if hourly.get("temperature_2m") else 25.0

                results.append({
                    "lat": batch[j]["lat"],
                    "lon": batch[j]["lon"],
                    "c_total": c_total if c_total is not None else 0,
                    "c_low": c_low if c_low is not None else 0,
                    "c_mid": c_mid if c_mid is not None else 0,
                    "c_high": c_high if c_high is not None else 0,
            ...
            # Fallback en caso de que la respuesta falle para este lote
            print(f"    [⚠] Usando valores por defecto para lote {batch_num}")
            for p in batch:
                results.append({
                    "lat": p["lat"],
                    "lon": p["lon"],
